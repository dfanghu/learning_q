...
def q_cut(cut_positions, original_list):
    return f"{cut_positions}_{original_list}"
...
def q_add_fold(x, y, space: bool = False) -> str:
    return q_binop(x, "+/", y, space=space)
...
def q_subtract_fold(x, y, space: bool = False) -> str:
    return q_binop(x, "-/", y, space=space)
...
def q_mul_fold(x, y, space: bool = False) -> str:
    return q_binop(x, "*/", y, space=space)
...
def q_div_fold(x, y, space: bool = False) -> str:
    return q_binop(x, "%/", y, space=space)


def q_square(x, space: bool = False) -> str:
    return q_mul(x, x)
